...
async function upsertCard(client, card) {
  ...
  const insertColumnsMatch = sql.match(/INSERT INTO public\.pt_cards\s*\(([\s\S]*?)\)\s*VALUES/i)
  const valuesMatch = sql.match(/VALUES\s*\(([\s\S]*?)\)\s*ON CONFLICT/i)
  const insertColumnCount = insertColumnsMatch
    ? insertColumnsMatch[1].split(',').filter(Boolean).length
    : 0
  const valuesPlaceholderCount = valuesMatch ? valuesMatch[1].split(',').filter(Boolean).length : 0

  console.log('UPSERT_CARD_VALUES_LEN', values.length)
  console.log('UPSERT_CARD_SQL_COLS', insertColumnCount)
  console.log('UPSERT_CARD_SQL_VALS', valuesPlaceholderCount)

  if (!insertColumnCount || !valuesPlaceholderCount) {
    throw new Error('pt_cards upsert could not parse SQL column/value counts')
  }

  if (insertColumnCount !== valuesPlaceholderCount || values.length !== valuesPlaceholderCount) {
    throw new Error(
      `pt_cards upsert mismatch: columns=${insertColumnCount} values=${valuesPlaceholderCount} bind=${values.length}`
    )
  }
