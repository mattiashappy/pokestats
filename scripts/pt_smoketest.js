...
const token = process.env.PT_API_TOKEN || process.env.PT_API_KEY || process.env.PT_API_BEARER || process.env.PRICE_TRACKER_TOKEN || process.env.PRICE_TRACKER_API_KEY;
if (!token) {
  console.error(\"No token found in env (PT_API_TOKEN/PT_API_KEY/PRICE_TRACKER_*)\");
  process.exit(1);
}
...
JS
node scripts/pt_smoketest.js'" -a pokestats
