...
function mapCanonicalExpansionsWithStats() {
  return CANONICAL_EXPANSIONS.map((expansion, index) => ({
    ...
    set_total: expansion.set_total ?? expansion.cards?.length ?? null,
...
function mapCanonicalCardsForSet(setCode) {
  const expansion = CANONICAL_EXPANSIONS.find(
    (exp) => exp.set_code.toLowerCase() === setCode.toLowerCase()
  )

  if (!expansion) return []

  return (expansion.cards ?? []).map((card, index) => ({
    id: index + 1,
    name: card.name,
    era: expansion.era ?? null,
    set_name: expansion.name ?? expansion.set_code,
    set_code: expansion.set_code,
    set_total: expansion.set_total ?? expansion.cards?.length ?? null,
    card_number: card.card_number,
    expansion_id: index + 1,
    linked_auctions: 0,
    last_seen: null
  }))
}
...
async function fetchCardsList({ setCode = null, expansionId = null } = {}) {
  if (!pool) {
    return setCode ? mapCanonicalCardsForSet(setCode) : []
  }
  ...
  try {
    ...
    console.error('Failed to fetch cards list, returning canonical fallback', error)
    return setCode ? mapCanonicalCardsForSet(setCode) : []
  }
}

async function fetchExpansionSummaries() {
  if (!pool) return mapCanonicalExpansionsWithStats()

  try {
    const infrastructureReady = await ensureCardInfrastructure()
    if (!infrastructureReady) return mapCanonicalExpansionsWithStats()
    ...
    if (result.rows.length === 0) return mapCanonicalExpansionsWithStats()
    return result.rows
  } catch (error) {
    console.error('Failed to fetch expansions, returning canonical fallback', error)
    return mapCanonicalExpansionsWithStats()
  }
}
